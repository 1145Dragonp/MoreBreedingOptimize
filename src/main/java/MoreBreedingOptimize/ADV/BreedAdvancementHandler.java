...
public class BreedAdvancementHandler {
    ...
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        if (animal.getPersistentData().getBoolean("morebo:neutered")) return;
...
// 6. 核心判断：如果成就存在，并且玩家已经完成了，就直接 return
        ...
        if (player.getServer() != null) {
            player.getServer().getCommands().performPrefixedCommand(
                    ...
                    "advancement grant @s only morebo:breed_with_end_rod"
            );
        }

        log.info("[MoreBreedingOptimize] Advancement '种公！' granted via command to player {}", player.getName().getString());
    }
}